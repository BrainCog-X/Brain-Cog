...
class Mask:
            ...
            if len(item.size()) > 1:
                if index<=10:
                    ...
                    if len(item.size()) == 4:
                        ...
                        if index+5==10:
                            self.mat[index+5][:,int(self.init_rate*item.size()[0])*49:]=0.0
                    if len(item.size()) == 2:
    ...
    def get_filter_codebook(self,index,ww,task,epoch): 
        ...
        if len(ww.size()) == 4:
            ...
            self.reduce[index]=self.reduce[index]*0.999+self.n_delta[index]*math.exp(-int((epoch-1)/12))
            ...
            for x in range(0, len(p_ind)):
                ...
                if index+5==10:
                    self.mat[index+5][:,p_ind[x]*49:(p_ind[x]+1)*49]=0
        ...
        for index, item in enumerate(self.model.parameters()):
            if len(item.size()) > 1 and index<10:
                ...
                for x in range(0, len(empy_nn)):
                    ...
                    if index+5==10:
                        self.mat[index+5][:,empy_nn[x]*49:(empy_nn[x]+1)*49]=0
